...
class AppDatabase extends _\$AppDatabase {
  ...
  Future<void> _seedCapabilities() async {
    final capabilities = [
      (ProviderType.deepseek, true, true, true, true, false, 'https://api.deepseek.com'),
      (ProviderType.mimo, false, true, true, true, true, 'https://api.mimo.ai/v1'),
      (ProviderType.gemini, false, true, true, true, true, 'https://generativelanguage.googleapis.com/v1beta'),
      (ProviderType.openrouter, true, true, true, true, false, 'https://openrouter.ai/api/v1'),
  ...
  Future<List<BalanceSnapshot>> getBalanceSnapshotsByAccount(int accountId, {int limit = 10}) {
  ...
  Future<int> insertUsageLog(UsageLogsCompanion log) => into(usageLogs).insert(log);
  ...
  Future<int> insertModelPrice(ModelPricesCompanion price) => into(modelPrices).insert(price);
  ...
  Future<int> insertAlertRule(AlertRulesCompanion rule) => into(alertRules).insert(rule);

  Future<String?> getSetting(String key) {
    return (select(appSettings)..where((t) => t.key.equals(key))).getSingleOrNull().then((row) => row?.value);
  }

  Future<int> setSetting(String key, String value) {
    return (update(appSettings)..where((t) => t.key.equals(key))).write(AppSettingsCompanion(value: Value(value)));
  }
  ...
  Future<int> cleanOldProxyRuntimeLogs(int retentionDays) {
    ...
    return (delete(proxyRuntimeLogs)..where((t) => t.createdAt.isSmallerThanValue(cutoff))).go();
  }
}
